...
public:
    ...
    // * Calculate the quadrupole moment for a node
    void calcQuadrupoleMoment() {
        ...
        for(auto particle : this->addresses) {
            ...
            this->Q += particle->m() * Tensor(relative_vector);
        }

    }

    // * Function which recursively calculates the quadrupole moment for every node except leaves
    void QuadrupoleMoment() {
        
        // * Check if it's a leaf
        if(this->N > 1) {
            this->calcQuadrupoleMoment();
            for(unsigned int i = 0; i < 8; i++) {
                if(this->octants[i]) {
                    this->octants[i]->QuadrupoleMoment();
                }
            }
        } 
    }
    ...
    Vec3d force(Vec3d relative_vector, double eps, bool leaf) {;
        if(leaf) {
            ...
            return ( (-1) * (this->mass * relative_vector) / (pow( hypot(eps, relative_vector.norm()), 3 )) ) 
            + (this->Q * relative_vector / (pow( hypot(eps, relative_vector.norm()), 5 )) )
            - (2.5 * relative_vector * (this->Q * relative_vector) * relative_vector / (pow( hypot(eps, relative_vector.norm()), 7 )) );
        }
    }

    // * Calculate the total force of a particle with softening
