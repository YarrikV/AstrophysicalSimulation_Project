...
public:
    ...
    // * Calculate the quadrupole integrals for a node
    void calcQuadrupoleIntegrals() {

        // * This variables represent the summation part of the integrals
        ...
        // * Summation
        for(auto particle : this->addresses) {
            relative_vector = particle->r() - this->barycentre;
            a0 += particle->m() * (3 * relative_vector.z() * relative_vector.z() - relative_vector.norm2());
            a1 -= particle->m() * hypot(relative_vector.x(), relative_vector.y()) * relative_vector.z(); // ! exponential
            a2 += particle->m() * (relative_vector.x() * relative_vector.x() + relative_vector.y() * relative_vector.y()); // ! exponential
            a_1 += particle->m() * hypot(relative_vector.x(), relative_vector.y()) * relative_vector.z(); // ! exponential
            a_2 += particle->m() * (relative_vector.x() * relative_vector.x() + relative_vector.y() * relative_vector.y()); // ! exponential
        }

        // * Adding the prefactors which already take in account the second spherical harmonic prefactors
        this->Q0 = 5 * M_1_PI * a0 / 16;
        this->Q1 = 15 * M_1_PI * a1 / 8;
        this->Q2 = 15 * M_1_PI * a2 / 32;
        this->Q_1 = 15 * M_1_PI * a_1 / 8;
        this->Q_2 = 15 * M_1_PI * a_2 / 32;
    }

    // * Function which recursively calculates the quadrupole integrals for every node except leaves
    void QuadrupoleIntegrals() {
        
        // * Check if node exists and if it's a leaf
        if(this->N > 1) {
            this->calcQuadrupoleIntegrals();
            for(unsigned int i = 0; i < 8; i++) {
                if(this->octants[i]) {
                    this->octants[i]->QuadrupoleIntegrals();
                }
            }
        } 
    }
    ...
    Vec3d force(Vec3d relative_vector, double eps, bool leaf) {
        if(leaf) {
            return (-1) * (this->mass * relative_vector) / (pow( hypot(eps, relative_vector.norm()), 3 ));
        }
        else {
            double quadrupole_moment = (3 * relative_vector.z() * relative_vector.z() / (relative_vector.norm() * relative_vector.norm()) - 1) * this->Q0
            - hypot(relative_vector.x(), relative_vector.y()) * relative_vector.z() / (relative_vector.norm() * relative_vector.norm()) * this->Q1 // ! exponential
            + (relative_vector.x() * relative_vector.x() + relative_vector.y() * relative_vector.y()) / (relative_vector.norm() * relative_vector.norm()) * this->Q2 // ! exponential
            + hypot(relative_vector.x(), relative_vector.y()) * relative_vector.z() / (relative_vector.norm() * relative_vector.norm()) * this->Q_1 // ! exponential
            + (relative_vector.x() * relative_vector.x() + relative_vector.y() * relative_vector.y()) / (relative_vector.norm() * relative_vector.norm()) * this->Q_2; // ! exponential
            
            return (-1) * relative_vector * (this->mass / (pow( hypot(eps, relative_vector.norm()), 3 )) 
            + 12 * M_PI * quadrupole_moment / (5 * (pow( hypot(eps, relative_vector.norm()), 5 ))));  
        }
    }
        ...
        if(this->N > 1 and this->addresses.find(elem) != this->addresses.end()) {
            ...
            if(this->N == 1) {
                ...
                if( (this->size / relative_vector.norm()) < delta ) {
                    ...
                    force = this->force(relative_vector, eps, false);
                }
                else {
