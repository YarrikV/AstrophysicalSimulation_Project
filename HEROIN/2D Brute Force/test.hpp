...
void leapFrog(vector<Particle>& particles, double eps, double h, double time_end, double N, string file_name) {
    ...
    for (unsigned int i = 0; i < N; i++) {
        v_half.push_back(Particle(Vec2d(), particles[i].v() + 0.5 * h * acc[i],
                                  particles[i].m()));
    }
    // for(unsigned int i = 0; i < N; i++) {
    ...
    for (double t = h; t <= time_end; t += h) {
        for(unsigned int i = 0; i < N; i++) {
            v_half[i].setVelocity(particles[i].v() + 0.5 * h * acc[i]);
            particles[i].addPosition(v_half[i].v() * h);
            acc = acceleration2d(v_half, N, eps);
            particles[i].setVelocity(v_half[i].v() + 0.5 * h * acc[i]);
        ...
        outfile_energy << t << ' ' << energy(particles, N, eps) << ' ' << error << endl;
    }
